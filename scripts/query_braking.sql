...
b4 AS (
	...
	CASE WHEN signal = 'brakeon' AND uit > aan THEN LAG(value, -1) OVER(PARTITION BY vin ORDER BY aan)
		WHEN signal = 'brakeoff' AND uit is NULL THEN LAG(value, 1) OVER(PARTITION BY vin ORDER BY aan)
		ELSE NULL
	END as v0,
...
c as (SELECT vin, aan, v0 as v1 FROM b4 WHERE signal = 'brakeoff')

-- voeg snelheid en tijdstip van rem-uit toe aan rem-aan o.b.v. timestamp
SELECT b4.vin, b4.aan as brakeon, b4.uit as brakeoff, b4.v0, c.v1, b4.dt, b4.geom FROM b4
