...
bk AS(
	SELECT 
		b.*, 
		CASE WHEN b.value - LAG(b.value, -1) OVER(ORDER BY auto, time) = 1 THEN LAG(b.time, -1) OVER(ORDER BY auto, time)
			ELSE NULL
			END as time2,
		CASE WHEN b.value - LAG(b.value, -1) OVER(ORDER BY auto, time) = 1 THEN 'brakeon'
			ELSE 'brakeoff'
			END as aanuit
	FROM b
) ,
...
joined as (
SELECT bk.time as time, bk.geom as geom1, brakeoff.geom as geom2 from bk JOIN brakeoff
ON bk.time2 = brakeoff.time
where bk.aanuit = 'brakeon')

SELECT j.time, ST_MAKELINE(ARRAY[j.geom1, j.geom2]) as geom, final.v0, final.v1, final.dt from joined j JOIN final
ON j.time = final.brakeon

WHERE ST_LENGTH(ST_MAKELINE(ARRAY[geom1, geom2])) > 0
